![AWS](https://img.shields.io/badge/AWS-Cloud-orange?logo=amazonaws)
![Linux](https://img.shields.io/badge/Linux-CLI-yellow?logo=linux)
![GitHub](https://img.shields.io/badge/GitHub-Open--Source-black?logo=github)

![Profile Views](https://komarev.com/ghpvc/?username=kevappbuilds&color=blue)
# 👋 Hey, I'm Kelvin
**Air Force Reserve | Aspiring Cloud, Cybersecurity & DevOps Engineer**  
Building, learning, and documenting everything publicly.
## 🚀 About Me
- ☁️ Aspiring Cloud & DevOps Engineer
- ✈️ Air Transportation (Air Force Reserve)
- 🐧 Learning Linux, AWS, and automation
- 🔐 Exploring cybersecurity fundamentals
- 📚 Documenting my journey one project at a time

## 🧰 Tech Stack
<div>
  <img src="https://skillicons.dev/icons?i=aws,linux,github,bash,docker,git" />
</div>
## 🛠️ Technologies I’m Learning
- **Cloud:** AWS (EC2, Lightsail, IAM, S3)
- **Linux:** CLI navigation, permissions, services, SSH
- **DevOps:** Git, GitHub, scripting, automation
- **Security:** Basic security principles & cloud best practices
## 📁 My Current Projects
- 🐧 `linux-basics` — commands, notes, hands-on practice
- ☁️ `aws-lightsail-notes` — server setup & cloud concepts
- 📘 `cloud-journey` — daily learning logs
- 🧪 `projects/` — practical labs and experiments
## 🎯 Goals for 2026
- Earn **AWS Cloud Practitioner**
- Build end-to-end cloud projects
- Learn **Terraform & CI/CD pipelines**
- Begin Cybersecurity training
- Land my first IT/Cloud job
## 📬 Connect With Me
I’m always learning—always building.
Feel free to reach out or follow my journey!
![Kelvin's GitHub Stats](https://github-readme-stats-github.vercel.app/api?username=kevappbuilds&show_icons=true&theme=tokyonight)